"""
Vendored from R2SAEA python_code/algorithm/util/edamodel.py（Lu et al. 2026）。
只保留 LSEA 用到的 local_search + VWH，去掉 matplotlib 依赖与未使用的 LBH。
函数体逐行照抄原文件（含其原有的数值容错处理），供 run_exp_bert.py 使用。
"""
...
class VWH(EDA_model):
    ...
    def update(self, Xs):
        ...
        epsilon = 1e-10
        index = np.floor(
            (self.M - 2) * (Xs - np.tile(self.LB, [NP, 1])) / (np.tile(self.UB - self.LB, [NP, 1]) + epsilon))
